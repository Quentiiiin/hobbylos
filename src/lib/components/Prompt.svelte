...
>
    ...
    {#if prompt.chosenAnswer !== undefined}
        <div
            class="mt-4 p-4 border-2 border-black rounded-md shadow-[2px_2px_0px_0px_rgba(0,0,0)] bg-white"
        >
            {prompt.question.answerTags[prompt.chosenAnswer]}
        </div>
    {:else}
        <div class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-4">
            {#each prompt.question.answerTags as answer, index}
                <button
